...
const dialToCountry = {
  ...
  30: "gr", 31: "nl", 32: "be", 33: "fr", 34: "es", 35: "reserved", 36: "hu",
  37: "reserved", 39: "it", 40: "ro", 41: "ch", 42: "cz", 43: "at", 44: "gb",
  45: "dk", 46: "se", 47: "no", 48: "pl", 49: "de", 50: "reserved",
  51: "pe", 52: "mx", 53: "cu", 54: "ar", 55: "br", 56: "cl", 57: "co", 58: "ve",
  59: "reserved", 60: "my", 61: "au", 62: "id", 63: "ph", 64: "nz", 65: "sg",
  66: "th", 67: "reserved", 68: "reserved", 69: "reserved",
  7: "ru", 81: "jp", 82: "kr", 84: "vn", 86: "cn",
  ...
  670: "tl", 672: "aq", 673: "bn", 674: "nr", 675: "pg", 676: "to", 677: "sb",
  ...
  967: "ye", 968: "om", 971: "ae", 972: "il", 973: "bh", 974: "qa", 975: "bt",
...
const dialCodeFromIso2 = (iso2) => {
  ...
  const entry = Object.entries(dialToCountry).find(([, cc]) => cc === iso2.toLowerCase());
  return entry ? String(entry[0]) : undefined;
};

const countryNameToIso2 = {
  India: "in", Australia: "au", "United States": "us", "United Kingdom": "gb",
  Canada: "ca", Singapore: "sg", "New Zealand": "nz", Germany: "de",
...
const normalizePhoneIntoTransfer = (condition, raw) => {
  ...
  const iso2 = parsed.country?.toLowerCase() || (dial && dialToCountry[dial]) || "us";
...
const mergeTransfersByCondition = (existing, incoming) => {
  ...
  incoming.forEach((t) => {
    const idx = next.findIndex(
      (x) => x.condition && x.condition.toLowerCase() === t.condition.toLowerCase()
    );
    if (idx >= 0) {
      next[idx] = {
        ...next[idx],
...
function CallTransfer() {
  ...
  useEffect(() => {
    ...
    if (!bizId) {
      setShowPopup(true);
      setPopupType("failed");
      setPopupMessage("No business selected. Missing 'SelectAgentBusinessId' in session.");
    ...
    (async () => {
      try {
        ...
        setShowPopup(true);
        setPopupType("failed");
        setPopupMessage("Failed to fetch business details. " + (error?.response?.data || error?.message || ""));
      } finally {
        setBizLoaded(true);
      }
    })();
  ...
  useEffect(() => {
    ...
    fetchLlmDetails(agentData?.agent?.llmId)
      .then((res) => {
        ...
        const billingT = normalizePhoneIntoTransfer("billing", vars?.billing_number);
        if (billingT) incoming.push(billingT);
        const supportT = normalizePhoneIntoTransfer("support", vars?.support_number);
  ...
  const handleRemove = async (index) => {
    const conditionKeyMap = { sales: "sales_number", billing: "billing_number", support: "support_number" };
    ...
    try {
      ...
      const key = removed?.condition && conditionKeyMap[(removed.condition || "").toLowerCase()];
      ...
      setShowPopup(true);
      setPopupType("failed");
      setPopupMessage("Failed to remove number: " + (err?.response?.data || err?.message || err));
    ...
    try {
      ...
      for (const t of transfers) {
        ...
        if (cond) {
          ...
          if (conditionCounts[cond] > 1) {
            setShowPopup(true);
            setPopupType("failed");
            setPopupMessage(`You cannot add multiple entries for the same department: '${cond}'.`);
            return;
          }
        }
      }
      ...
      for (const [i, t] of transfers.entries()) {
        ...
        if (!phone || !dialCode) {
          setShowPopup(true);
          setPopupType("failed");
          setPopupMessage(`Phone number and dial code are required for entry ${i + 1}.`);
          return;
        }
        ...
        if (!parsed || parsed.country?.toLowerCase() !== t.countryCode) {
          ...
          setPopupType("failed");
          setPopupMessage(
            `Invalid phone number for entry ${i + 1}. Ensure it matches +${t.dialCode} (${t.countryCode.toUpperCase()}).`
          );
          return;
        }
      }
      ...
      const supportEntry = transfers.find((t) => (t.condition || "").toLowerCase() === "support");
      ...
      setShowPopup(true);
      setPopupType("failed");
      setPopupMessage("Failed to update LLM: " + (error?.response?.data || error.message || error));
  ...
  return (
    <>
      ...
      <div className={styles.CallTransferMainDiv} style={{ opacity: initializing ? 0.4 : 1 }}>
        ...
        <div className={styles.CallTransferMain}>
          <div className={styles.headrPart}>
            ...
            <img
              ...
              style={{
                cursor: "pointer",
                opacity: transfers.some(t => !(t.condition||"").trim() && !(t.phone||"").trim()) ? 0.5 : 1,
                pointerEvents: transfers.some(t => !(t.condition||"").trim() && !(t.phone||"").trim()) ? "none" : "auto",
              }}
            />
          </div>
          ...
          {transfers.map((item, index) => (
            <div
              ...
              style={{ marginBottom: index === transfers.length - 1 ? "5rem" : undefined }}
            >
              <div className={styles.selectWrapper}>
                ...
                <select
                  ...
                  onChange={(e) => updateTransfer(index, { condition: e.target.value })}
                >
                ...
                <img src="svg/select-arrow.svg" alt="arrow" className={styles.arrowIcon} />
              </div>

              <label className={styles.label}>Forward to</label>
              <div className={styles.phoneInput}>
                <PhoneInput
                  country={item.countryCode}  // already correct from business defaults
                  enableSearch
                  value={item.phone}
                  onChange={(val, c) => {
                    const dialCode = c?.dialCode || item.dialCode;
                    const countryCode = (c?.countryCode || item.countryCode).toLowerCase();
                    const digits = (val || "").replace(/\D/g, "");
                    const normalized = digits.startsWith(dialCode) ? digits : dialCode + digits;
                    updateTransfer(index, { phone: normalized, dialCode, countryCode });
                  }}
